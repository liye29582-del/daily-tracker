...
export default function Stats() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">统计</h1>
      ...
      <div className="flex bg-gray-100 rounded-full p-1 mb-4 w-fit">
        {([['week', '本周'], ['month', '本月']] as const).map(([v, l]) => (
          <button
            key={v}
            onClick={() => setScope(v)}
            className={`px-5 py-1.5 rounded-full text-sm ${scope === v ? 'bg-white text-brand-700 shadow-sm font-medium' : 'text-gray-500'}`}
          >
            {l}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-3xl p-6 bg-gradient-to-br from-emerald-50 to-emerald-100/60 border border-white/70 shadow-sm">
          ...
          <div className={`text-3xl font-bold tabular-nums ${lost != null && lost > 0 ? 'text-emerald-600' : lost != null && lost < 0 ? 'text-rose-500' : 'text-gray-700'}`}>
            {lost != null ? (lost > 0 ? `-${lost}` : lost < 0 ? `+${Math.abs(lost)}` : '0') : '—'}
            {lost != null && <span className="text-base font-medium text-gray-400 ml-1">kg</span>}
          </div>
          <div className="text-sm text-gray-500 mt-1">{lost != null && lost < 0 ? '累计增重' : '累计已减'}</div>
        </div>
        <div className="rounded-3xl p-6 bg-gradient-to-br from-sky-50 to-sky-100/60 border border-white/70 shadow-sm">
          ...
          <div className="text-sm text-gray-500 mt-1">完成待办</div>
        </div>
        <div className="rounded-3xl p-6 bg-gradient-to-br from-amber-50 to-amber-100/60 border border-white/70 shadow-sm">
          ...
          <div className="text-sm text-gray-500 mt-1">累计专注</div>
        </div>
      </div>
      ...
      <div className="mt-4 bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between text-sm mb-2">
          <span className="text-gray-500">待办完成率</span>
        ...
        <div className="text-xs text-gray-400 mt-2">本{scope === 'week' ? '周' : '月'}共 {totalN} 条待办，已完成 {doneN} 条</div>
      </div>

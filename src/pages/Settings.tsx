  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm space-y-4">
        <div>
          <label className="text-sm text-gray-500">昵称（可选）</label>
          <input
            value={settings.nickname ?? ''}
            onChange={(e) => saveSettings({ nickname: e.target.value.trim() || undefined })}
            placeholder="怎么称呼你？"
            className="w-full mt-1 border border-gray-200 rounded-lg px-3 py-2 outline-none focus:border-brand-400"
          />
        </div>
        <div>
            ...
            {([['male', '男'], ['female', '女'], ['other', '其他']] as const).map(([v, l]) => (
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-sm border ${
                  settings.gender === v ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-gray-600 border-gray-200'
                }`}
              >
                {l}
              </button>
        ...
        <div>
          <label className="text-sm text-gray-500">身高（厘米，选填）</label>
          <input
            type="number"
            min={100}
            max={250}
            ...
            className="w-full mt-1 border border-gray-200 rounded-lg px-3 py-2 outline-none focus:border-brand-400"
          />
          <div className="text-[11px] text-gray-400 mt-1">用于计算 BMI 指数</div>
        </div>
        <div>
          <label className="text-sm text-gray-500">专注时长（分钟）</label>
          <input
            type="number"
            min={1}
        ...
        <div>
          <label className="text-sm text-gray-500">休息时长（分钟）</label>
          <input
            type="number"
            min={1}
        ...
        <div>
          <label className="text-sm text-gray-500">目标体重（kg，可选）</label>
          <input
            type="number"
            step="0.1"
        ...
        <div>
          <label className="text-sm text-gray-500">每日喝水目标（杯，1 杯≈250ml）</label>
          <input
            type="number"
            min={1}
      ...
      <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm mt-4 space-y-3">
        <div className="text-sm font-medium">数据备份</div>
        <p className="text-xs text-gray-400">所有数据仅存于本机浏览器。导出后可换设备或留存。</p>
        <div className="flex gap-3">
          <button onClick={doExportJSON} className="flex-1 bg-brand-600 text-white py-2.5 rounded-lg text-sm font-medium">
            导出全部 (JSON)
          </button>
          <button onClick={doExportCSV} className="flex-1 bg-gray-100 text-gray-700 py-2.5 rounded-lg text-sm font-medium">
            导出体重 (CSV)
          </button>
        </div>
      </div>
